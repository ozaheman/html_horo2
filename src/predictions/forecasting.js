...
window.PREDICTION_FORECASTING = {
  ...
  getCurrentDashaInfo: function(targetDate = new Date()) {
    ...
    // Set to mid-day to avoid boundary issues with birth time in first dasha
    const d = new Date(targetDate);
    d.setHours(12, 0, 0, 0);
    const jd = this.dateToJD(d);
    ...
    return {
      ...
      prana: vimshotariDasha?.prana || null,
  ...
  findActiveDasha: function(dashaArray, jd) {
    ...
    const targetJD = (typeof jd === 'number') ? jd : this.dateToJD(jd);
    
    // Find Mahadasha
    const maha = dashaArray.find(d => {
      const start = (d.start instanceof Date) ? this.dateToJD(d.start) : d.start;
      const end = (d.end instanceof Date) ? this.dateToJD(d.end) : d.end;
      return targetJD >= start && targetJD <= end;
    });

    if (!maha) return null;

    const result = { maha, ant: null, prat: null, suksh: null, prana: null };

    // Helper to find sub in array
    const findSub = (arr) => arr ? arr.find(s => {
      const sStart = (s.start instanceof Date) ? this.dateToJD(s.start) : s.start;
      const sEnd = (s.end instanceof Date) ? this.dateToJD(s.end) : s.end;
      return targetJD >= sStart && targetJD <= sEnd;
    }) : null;

    result.ant = findSub(maha.subs);
    if (result.ant) result.prat = findSub(result.ant.subs);
    if (result.prat) result.suksh = findSub(result.prat.subs);
    if (result.suksh) result.prana = findSub(result.suksh.subs);
