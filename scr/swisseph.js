...
class SwissEph {
  ...
  async initSwissEph() {
    ...
    // Check if we are running from a local file:// URL (offline mode)
    const isOffline = (typeof window !== 'undefined' && window.location && window.location.protocol === 'file:');
    
    if (isOffline) {
      console.log('Running in offline mode (file://), using embedded base64 data.');
      try {
        const { wasmBase64, dataBase64 } = await import('./swisseph_offline_data.js');
        moduleConfig.wasmBinary = decodeBase64(wasmBase64);
        moduleConfig.getPreloadedPackage = function(remotePackageName, remotePackageSize) {
          return decodeBase64(dataBase64).buffer;
        };
      } catch (e) {
        console.error("Failed to load offline data payload. Ensure swisseph_offline_data.js is generated.", e);
      }
    } else {
