...
function renderYogasDashboard(yogasData = [], summary = {}) {
    ...
    const calculatedSummary = {
        ...
        primaryYogas: detectedYogas.slice(0, 5)
    };
...
function renderSummarySection(summary = {}) {
    ...
    let html = `
        <div class="yoga-summary-section">
            ...
            <div style="margin-top: 15px;">
                <div style="font-weight: bold; color: #333; margin-bottom: 8px;">Yoga Index: ${index.interpretation || 'N/A'}</div>
                <div class="yoga-index-indicator"></div>
                <div style="font-size: 12px; color: #666;">
                    ${index.positiveScore || 0} Positive Points - ${index.negativeScore || 0} Challenging Points = 
                    <span style="color: ${index.score > 0 ? '#4caf50' : '#f44336'}; font-weight: bold;">
    ...
    if (summary.primaryYogas && summary.primaryYogas.length > 0) {
        html += `<div style="margin-top: 20px; padding: 15px; background: #fff3cd; border-left: 4px solid #FFD700; border-radius: 5px;">
            <div style="margin-bottom: 15px;">
                <strong style="font-size: 14px; color: #333;">✨ Top Detected Yogas:</strong>
            </div>`;
        summary.primaryYogas.forEach(yoga => {
            html += `<span style="display: inline-block; margin-right: 12px; margin-bottom: 8px; padding: 6px 12px; background: white; border-radius: 20px; font-size: 12px;">
                🔹 ${yoga.name} <span style="color: #FFD700; font-weight: bold;">(${yoga.strength})</span>
            </span>`;
        });
        html += '</div>';
    }
    ...
    let html = `
        <div class="yoga-tile ${quality}" data-yoga-name="${yoga.name}" data-yoga-quality="${quality}" ${referenceAttr} ${displayStyle}>
            ...
            <span class="yoga-strength-badge ${strengthClass}">
                💪 ${yoga.strength?.level || 'Moderate'} Strength
            </span>
